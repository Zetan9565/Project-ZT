...
public class PlantManager : SingletonMonoBehaviour<PlantManager>, IWindowHandler
{
    ...
    public Canvas SortCanvas => UI && UI.gameObject ? UI.windowCanvas : null;
    ...
    public void Init(Field field)
    {
        ...
        int numToAdd = seeds.Count - SeedAgents.Count;
        for (int i = 0; i < numToAdd; i++)
        {
            SeedAgent sa = ObjectPool.Instance.Get(UI.seedCellPrefab, UI.seedCellsParent).GetComponent<SeedAgent>();
            SeedAgents.Add(sa);
        }
        for (int i = seeds.Count - 1; i >= 0 && i < SeedAgents.Count; i++)
            SeedAgents[i].Clear(true);
        SeedAgents.RemoveAll(x => !x.Seed);
    ...
    public void Search()
    {
        ...
        if (string.IsNullOrEmpty(name)) ShowAll();
                ...
                if (!sa.name.Contains(name))
                    ZetanUtil.SetActive(sa.gameObject, false);
    }
    ...
    private void ShowAll()
    {
        foreach (var sa in SeedAgents)
            ZetanUtil.SetActive(sa.gameObject, true);
    }
    ...
    public void OpenWindow()
    {
        ...
        if (IsUIOpen) return;
        if (IsPausing) return;
        UI.plantWindow.alpha = 1;
        UI.plantWindow.blocksRaycasts = true;
        IsUIOpen = true;
    ...
    public void CloseWindow()
    {
        ...
        if (!IsUIOpen) return;
        if (IsPausing) return;
        UI.plantWindow.alpha = 0;
        UI.plantWindow.blocksRaycasts = false;
        IsUIOpen = false;
