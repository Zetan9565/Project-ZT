    ...
    public class CraftWindow : Window, IHideable
    {
        ...
        private void CraftCurrent()
        {
            ...
            if (!Manualy(craftable))
            {
                ...
                {
                    ConfirmWindow.StartConfirm(Tr("确定制作1次 [{0}] 吗？", ItemFactory.GetColorName(currentItem)), delegate
                    {
                        IsCrafting = true;
                ...
                else
                {
                    ...
                    {
                        ConfirmWindow.StartConfirm(Tr("确定制作{0}次 [{1}] 吗？", (int)amount, ItemFactory.GetColorName(currentItem)), delegate
                        {
                            int num = (int)amount;
                            IsCrafting = true;
                            WindowsManager.HideWindow(this, true);
                    ...
                    }, amountCanCraft, Tr("制作次数"), Utility.ScreenCenter, Vector2.zero);
                }
            }
            else
            {
        ...
        public void ShowDescription(Item item)
        {
            ...
            nameText.text = ItemFactory.GetColorName(item);
            craftButton.interactable = craftAmount > 0;
            descriptionWindow.alpha = 1;
            descriptionWindow.blocksRaycasts = true;
            descOpen = true;
            Utility.SetActive(loopToggle.gameObject, Manualy(craftable));
        }
