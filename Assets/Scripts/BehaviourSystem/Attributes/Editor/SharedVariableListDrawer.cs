...
using UnityEngine;
using ZetanStudio.BehaviourTree.Editor;
...
namespace ZetanStudio.BehaviourTree
{
    public sealed class SharedVariableListDrawer
    {
        ...
        public SharedVariableListDrawer(SerializedProperty serializedVariables, bool isShared)
        {
            SerializedObject serializedObject = serializedVariables.serializedObject;
            variableList = new ReorderableList(serializedObject, serializedVariables, true, true, true, true)
            {
                drawElementCallback = (rect, index, isFocused, isActive) =>
                {
                    if (serializedObject.targetObject)
                    {
                        ...
                        EditorGUI.PropertyField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), serializedVariables.GetArrayElementAtIndex(index), true);
        ...
        {
            var settings = BehaviourTreeEditorSettings.GetOrCreate();
    ...
    public sealed class SharedVariablePresetListDrawer
    {
        ...
        public SharedVariablePresetListDrawer(SerializedProperty presetVariables, ISharedVariableHandler variableHandler, Func<int, Type> elementTypeGetter)
        {
            float lineHeight = EditorGUIUtility.singleLineHeight;
            float lineHeightSpace = lineHeight + EditorGUIUtility.standardVerticalSpacing;
            SerializedObject serializedObject = presetVariables.serializedObject;
            presetVariableList = new ReorderableList(serializedObject, presetVariables, true, true, true, true)
            {
                drawElementCallback = (rect, index, isFocused, isActive) =>
                {
                    ...
                    variable.isExpanded = EditorGUI.Foldout(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), variable.isExpanded,
                        new GUIContent(string.IsNullOrEmpty(name.stringValue) ? $"({type.Name})" : name.stringValue), true);
