...
namespace ZetanStudio.QuestSystem.Editor
{
    public class QuestEditor : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            ...
            list = root.Q<ListView>("quest-list");
            list.makeItem = () =>
            {
                var label = new Label();
            ...
            list.onSelectionChange += (o) =>
            {
                if (o != null && o.FirstOrDefault() is not null)
                {
                    ...
                    if (data.group) editor = UnityEditor.Editor.CreateEditor(data.group);
                    else editor = UnityEditor.Editor.CreateEditor(data.quest);
        ...
        {
            var groupTemp = Utility.Editor.LoadAssets<QuestGroup>();
            datas.Clear();
        ...
        {
            var questTemp = Utility.Editor.LoadAssets<Quest>();
            datas.Clear();
        ...
        private void OnTabMenu(int index, ContextualMenuPopulateEvent evt)
        {
            switch (index)
            {
                case 1:
                    ...
                    {
                        Quest quest = Utility.Editor.SaveFilePanel(CreateInstance<Quest>, ping: true);
                        if (quest)
                        {
                ...
                case 2:
                    ...
                    {
                        QuestGroup group = Utility.Editor.SaveFilePanel(CreateInstance<QuestGroup>, ping: true);
                        if (group)
                        {
