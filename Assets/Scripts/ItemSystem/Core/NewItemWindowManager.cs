...
public class NewItemWindowManager : WindowHandler<NewItemWindowBaseUI, NewItemWindowManager>
{
    ...
    public override void SetUI(NewItemWindowBaseUI UI)
    {
        this.UI = UI;
    ...
    private Stack<Text> titleCache;
    private Stack<Text> contentCache;
    private Stack<RoleAttributeAgent> attrCache;

    private int lineCount;

    public void ShowItemInfo(ItemAgent itemAgent)
    {
        if (MItemInfo.Equals(itemAgent.MItemInfo)) return;
        MItemInfo = itemAgent.MItemInfo;
        UI.icon.overrideSprite = MItemInfo.item.Icon;
        UI.nameText.text = MItemInfo.ItemName;
        UI.nameText.color = GameManager.QualityToColor(MItemInfo.item.Quality);
    ...
    private void PushTitle(string content)
    {
        ...
        if (titleCache.Count < 1)
            find = ObjectPool.Get(UI.titlePrefab, UI.contentParent).GetComponent<Text>();
    ...
    private void PushContent(string content)
    {
        Text find;
        if (titleCache.Count < 1)
            find = ObjectPool.Get(UI.contentPrefab, UI.contentParent).GetComponent<Text>();
