...
namespace ZetanStudio.ItemSystem.Editor
{
    ...
    public class ItemSelectorDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                if (fieldInfo.GetCustomAttribute<ItemFilterAttribute>() is not ItemFilterAttribute filter) items ??= Item.Editor.GetItems();
                else items ??= Item.Editor.GetItemsWhere(filter.DoFilter);
                Draw(position, property, label, () => items);
            }
            else EditorGUI.PropertyField(position, property, label);
        }

        public static void Draw(Rect rect, SerializedProperty property, GUIContent label, Func<IEnumerable<Item>> itemsGetter, Action<Item> selectCallback = null, ItemFilterAttribute itemFilter = null)
        {
            ...
            {
                var dropdown = new AdvancedDropdown<Item>(items, selectCallback: selectCallback,
                ...
                void addCallback()
                {
                    property.objectReferenceValue = ItemEditor.OpenAndCreateItem(itemFilter);
                    property.serializedObject.ApplyModifiedProperties();
                }
            }
            EditorGUI.EndProperty();
            ...
            static void OnPropertyContextMenu(GenericMenu menu, SerializedProperty property)
            {
                ...
                {
                    ItemEditor.CreateWindow(property.objectReferenceValue as Item);
                });
            }
        }
    }
