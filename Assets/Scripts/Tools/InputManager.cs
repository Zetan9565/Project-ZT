    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(customInfo.InteractiveButton))
        {
            if (DialogueManager.Instance.TalkAble)
            {
                ...
                else if (DialogueManager.Instance.DialogueType == DialogueType.Normal && DialogueManager.Instance.OptionsCount < 1 && DialogueManager.Instance.HasNotAcptQuests)
...
#elif UNITY_ANDROID
        if (Input.GetButtonDownMobile("Interactive"))
        {
            if (DialogueManager.Instance.TalkAble)
            {
                if (!DialogueManager.Instance.IsTalking)
