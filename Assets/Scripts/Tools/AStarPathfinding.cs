            ...
            else
            {
                switch (castCheckType)
                {
                    ...
                    case CastCheckType.Capsule:
                        ...
                        break;
                    case CastCheckType.Ray:
                        node.walkable = !Physics.Raycast(node.worldPosition + Vector3.up * (cellSize * (cellHeight - 1) + cellSize / 2 + 0.01f), Vector3.down,
                            cellSize * cellHeight, unwalkableLayer, QueryTriggerInteraction.Ignore) &&
                            !Physics.Raycast(node.worldPosition + Vector3.up * (cellSize * (cellHeight - 1) + cellSize / 2 + 0.01f), Vector3.up,
                            cellSize * cellHeight, unwalkableLayer, QueryTriggerInteraction.Ignore);
                        break;
                }
            }
