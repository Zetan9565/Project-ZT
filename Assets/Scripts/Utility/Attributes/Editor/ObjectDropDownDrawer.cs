...
public class ObjectDropDownDrawer : PropertyDrawer
{
    ...
    {
        objects ??= ZetanUtility.Editor.LoadAssets(type, resPath);
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsName, ZetanUtility.CommonBindingFlags) is FieldInfo field) name = field.GetValue(obj).ToString();
                else if (obj.GetType().GetProperty(memberAsName, ZetanUtility.CommonBindingFlags) is PropertyInfo property) name = property.GetValue(obj).ToString();
                else if (obj.GetType().GetMethod(memberAsName, ZetanUtility.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Length < 1)
