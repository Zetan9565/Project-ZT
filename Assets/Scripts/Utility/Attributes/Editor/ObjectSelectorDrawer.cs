...
namespace ZetanStudio.Editor
{
    ...
    public class ObjectSelectorDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                objects ??= Utility.Editor.LoadAssets(type, attribute?.resPath, attribute?.extension, attribute?.ignorePackages ?? true);
                if (attribute != null)
                {
                                                       ...
                                                       string title, bool displayNone, bool displayAdd, System.Type type) where T : Object
        {
            ...
            var dropdown = new AdvancedDropdown<T>(objects, i => { property.objectReferenceValue = i; property.serializedObject.ApplyModifiedProperties(); },
                                                   i => GetName(memberAsName, i), i => GetGroup(memberAsGroup, i), Utility.Editor.GetIconForObject,
        ...
        private static void Draw(System.Type type, System.Func<AdvancedDropdown> getDropdown, Rect position, SerializedProperty property, GUIContent label, string memberAsName, string memberAsTooltip, string nameNull)
        {
            ...
            if (EditorGUI.DropdownButton(buttonRect,
                                         new GUIContent(name)
                                         {
                                             image = Utility.Editor.GetIconForObject(property.objectReferenceValue),
        ...
        {
            var obj = Utility.Editor.SaveFilePanel(() => ScriptableObject.CreateInstance(type), assetName, ping: true);
            if (obj)
            {
        ...
        private static string GetName(string memberAsName, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsName, Utility.CommonBindingFlags) is FieldInfo field)
                {
                    name = field.GetValue(obj)?.ToString();
                    break;
                }
                else if (obj.GetType().GetProperty(memberAsName, Utility.CommonBindingFlags) is PropertyInfo property)
                {
                    name = property.GetValue(obj)?.ToString();
                    break;
                }
                else if (obj.GetType().GetMethod(memberAsName, Utility.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Where(x => !x.IsOptional).Count() < 1)
                {
        ...
        private static string GetGroup(string memberAsGroup, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsGroup, Utility.CommonBindingFlags) is FieldInfo field) group = field.GetValue(obj).ToString();
                else if (obj.GetType().GetProperty(memberAsGroup, Utility.CommonBindingFlags) is PropertyInfo property) group = property.GetValue(obj).ToString();
                else if (obj.GetType().GetMethod(memberAsGroup, Utility.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Where(x => !x.IsOptional).Count() < 1)
        ...
        private static string GetTooltip(string memberAsName, string memberAsTooltip, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsTooltip, Utility.CommonBindingFlags) is FieldInfo field) tooltip = field.GetValue(obj).ToString();
                else if (obj.GetType().GetProperty(memberAsTooltip, Utility.CommonBindingFlags) is PropertyInfo property) tooltip = property.GetValue(obj).ToString();
                else if (obj.GetType().GetMethod(memberAsTooltip, Utility.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Length < 1)
