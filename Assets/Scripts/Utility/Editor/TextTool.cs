                ...
                case 0:
                    ...
                    if (GUILayout.Button("一键收集"))
                    {
                        ...
                        switch (type)
                        {
                            case ConvertType.Legacy:
                                texts.Clear();
                                if (onlyOpenScene && Utility.GetActiveScene() == null)
                                    Debug.LogWarning("没有打开的场景");
                                else if (Utility.GetActiveScene() != null)
                                {
                                    foreach (var root in Utility.GetActiveScene().GetRootGameObjects())
                                    {
                                ...
                                if (!onlyOpenScene)
                                {
                                    foreach (var go in Utility.Editor.LoadAssets<GameObject>(path))
                                    {
