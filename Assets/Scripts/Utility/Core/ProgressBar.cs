        ...
        public void New(float seconds, Action doneAction, Action cancelAction, string actionName = null, bool displayCancel = false)
        {
            ...
            if (actionText)
            {
                if (string.IsNullOrEmpty(actionName))
                    Utility.SetActive(actionText.gameObject, false);
                else
                {
                    actionText.text = actionName;
                    Utility.SetActive(actionText.gameObject, true);
                }
            }
            loopTimes = 0;
            Utility.SetActive(bar, true);
            Utility.SetActive(cancel, displayCancel);
