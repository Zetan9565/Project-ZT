...
namespace ZetanStudio.InteractionSystem.UI
{
    public class InteractionPanel : SingletonWindow<InteractionPanel>, IHideable
    {
        ...
        public bool Insert(IInteractive interactive)
        {
            ...
            if (buttonParent.childCount < 2) selectIndex = button.transform.GetSiblingIndex();
            UpdateButtons();
            UpdateView();
            return true;
        }
        ...
        public bool Remove(IInteractive interactive)
        {
            if (buttons.TryGetValue(interactive, out var button))
            {
                ...
                if (buttonParent.rect.height >= view.rect.height)
                    buttonParent.anchoredPosition += Vector2.up * button.GetComponent<RectTransform>().rect.height;
                else buttonParent.anchoredPosition = Vector2.zero;
        ...
        {
            if (selectIndex > buttonParent.childCount - 1) selectIndex = buttonParent.childCount - 1;
        ...
        {
            if (selectIndex >= buttonParent.childCount) return;
            LayoutRebuilder.ForceRebuildLayoutImmediate(buttonParent);
