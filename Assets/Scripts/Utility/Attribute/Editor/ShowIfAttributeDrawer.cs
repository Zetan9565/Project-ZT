...
public class ShowIfAttributeDrawer : PropertyDrawer
{
    ...
    private bool ShouldShow(ShowIfAttribute hideAttr, SerializedProperty property)
    {
        if (hideAttr.paths.Length < 1)
            return false;
        if (hideAttr.type != null) return ShouldShow0();
        else if (string.IsNullOrEmpty(hideAttr.relational)) return ShouldShow1();
        else return ShouldShow2();
        ...
        bool ShouldShow1()
        {
            bool hide = ShouldShow(hideAttr.paths[0], hideAttr.values[0]);
            for (int i = 1; i < hideAttr.paths.Length; i++)
            {
                if (hideAttr.and) hide &= ShouldShow(hideAttr.paths[i], hideAttr.values[i]);
                else hide |= ShouldShow(hideAttr.paths[i], hideAttr.values[i]);
            }
            return hide;
        }
                ...
                foreach (var item in RPN)
                {
                    ...
                    if (int.TryParse(item, out int index))
                    {
                        if (index >= 0 && index < hideAttr.paths.Length)
                            values.Push(ShouldShow(hideAttr.paths[index], hideAttr.values[index]));
                        else
                        {
                            Debug.Log("return 1");
                            return true;
