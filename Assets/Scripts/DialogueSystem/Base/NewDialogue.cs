...
{
    [CreateAssetMenu]
    public sealed class NewDialogue : ScriptableObject
    {
        ...
        public NewDialogue()
        {
            contents = new DialogueContent[] { new EntryContent() };
        }
        ...
        {
            public static DialogueContent AddContent(NewDialogue dialogue, Type type)
            {
            ...
            public static DialogueContent RemoveContent(NewDialogue dialogue, DialogueContent content)
            {
                UnityEditor.ArrayUtility.Remove(ref dialogue.contents, content);
                return content;
            }
            public static void CopyFromOld(NewDialogue dialogue, Dialogue old)
            {
                ...
                for (int i = 0; i < old.Words.Count; i++)
                {
                    ...
                    if (i == 0) parent = content = addContent(new EntryContent(talker, words.Content));
                    else
                    {
                            ...
                            switch (oopt.OptionType)
                            {
                                ...
                                case WordsOptionType.Choice:
                                    if (oopt.DeleteWhenCmplt)
                                    {
                                        ...
                                        DialogueOption.Editor.SetContent(delete.FirstOption,
            ...
            public static string Preview(NewDialogue dialogue)
            {
                ...
                foreach (var content in dialogue.contents)
                {
                    ...
                    {
                        sb.Append(Keywords.Editor.HandlingKeyWords(text.Talker));
                        sb.Append(": ");
                        sb.Append(Keywords.Editor.HandlingKeyWords(text.Text));
                        if (dialogue.contents[^1] != content) sb.Append('\n');
                    }
                    else if (content is BranchContent branch && branch.Dialogue)
                    {
                        sb.Append(Keywords.Editor.HandlingKeyWords(branch.Dialogue.Entry.Talker));
                        sb.Append(": ");
                        sb.Append(Keywords.Editor.HandlingKeyWords(branch.Dialogue.Entry.Text));
                        if (dialogue.contents[^1] != content) sb.Append('\n');
                    }
                }
                return sb.ToString();
