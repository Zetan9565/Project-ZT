                ...
                bool searchCon(Dialogue dialogue, out string content, out string tooltip)
                {
                    ...
                    foreach (var con in dialogue.Nodes)
                    {
                        if (con is SentenceNode sentence)
                            ...
                            {
                                content = $"{dialogue.name}\n({Tr("内容讲述人")}: {Utility.Editor.HighlightKeyword(sentence.Talker, keywords, 30)})";
                                tooltip = $"{dialogue.name}\n({Tr("内容讲述人")}: {sentence.Talker})";
                                return true;
                            }
                            ...
                            {
                                content = $"{dialogue.name}\n({Tr("内容文字")}: {Utility.Editor.HighlightKeyword(sentence.Text, keywords, 30)})";
                                tooltip = $"{dialogue.name}\n({Tr("内容文字")}: {sentence.Text})";
                                return true;
                            }
            ...
            inspector.onGUIHandler = () =>
            {
                if (node.SerializedNode?.serializedObject?.targetObject)
                {
                    ...
                    if (node.Target is IEventNode)
                        if (GUILayout.Button(Tr("查看事件"))) InspectEvents(node.SerializedNode.FindPropertyRelative("events"));
                }
            };
        }
