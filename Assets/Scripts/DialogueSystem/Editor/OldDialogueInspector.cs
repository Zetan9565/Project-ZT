...
[CustomEditor(typeof(OldDialogue))]
public class OldDialogueInspector : Editor
{
    ...
    private void OnEnable()
    {
        ...
        dialogueCache = ZetanUtility.Editor.LoadAssets<OldDialogue>().ToArray();
        ...
        dialogue = target as OldDialogue;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (string.IsNullOrEmpty(_ID.stringValue) || OldDialogue.Editor.IsIDDuplicate(dialogue, dialogueCache))
        {
            if (!string.IsNullOrEmpty(_ID.stringValue) && OldDialogue.Editor.IsIDDuplicate(dialogue, dialogueCache)) EditorGUILayout.HelpBox("此识别码已存在！", MessageType.Error);
            else EditorGUILayout.HelpBox("识别码为空！", MessageType.Error);
            if (GUILayout.Button("自动生成识别码"))
            {
                _ID.stringValue = OldDialogue.Editor.GetAutoID(dialogueCache);
                            ...
                            if (option.isExpanded)
                            {
                                ...
                                if (!(optionType.intValue == (int)WordsOptionType.Choice && !hasWordsToSay.boolValue))
                                {
                                    if (optionType.intValue == (int)WordsOptionType.BranchDialogue)
                                    {
                                        ...
                                        _lineCount++;
                                        if ((dialogue.objectReferenceValue as OldDialogue) == this.dialogue)
                                        {
