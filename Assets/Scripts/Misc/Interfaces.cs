...
public interface IScaleAble<T> where T : MonoBehaviour
{
    ...
    static void ScaleTo(IScaleAble<T> scaler, Vector3 scale, float duration, Action onDone = null)
    {
        ...
        scaler.ScaleCoroutine = scaler.MonoBehaviour.StartCoroutine(scaleTo(scaler.MonoBehaviour.transform, scale, duration, onDone));

        static IEnumerator scaleTo(Transform target, Vector3 scale, float duration, Action onDone)
        {
            float time = 0;
            while (time < duration)
            {
