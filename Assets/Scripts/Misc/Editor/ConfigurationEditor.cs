...
namespace ZetanStudio.Editor
{
    public abstract class ConfigurationEditor<T> : EditorWindow where T : ScriptableObject
    {
        ...
        {
            results.Clear();
            objects = Utility.Editor.LoadAssets<T>();
            ...
            if (objectsList == null)
            {
                ...
                objectsList.drawElementCallback = (rect, index, isActive, isFocused) =>
                {
                    ...
                    selection:
                        ...
                        {
                            if (Utility.Editor.IsValidFolder(folder))
                            {
                                try
                                {
                                    AssetDatabase.MoveAsset(AssetDatabase.GetAssetPath(GetResult(objectsList.list[index]).find), $"{Utility.Editor.ConvertToAssetsPath(folder)}/{Utility.GetFileName(AssetDatabase.GetAssetPath(GetResult(objectsList.list[index]).find))}");
                                    Refresh();
                                    latestFolder = folder;
                                }
                                catch
                ...
                objectsList.drawHeaderCallback = (rect) =>
                {
                    ...
                    GUIStyle style = Utility.Editor.Style.middleRight;
        ...
        protected virtual bool CompareKey(T element, out string remark)
        {
            ...
            {
                remark = "名称：" + Utility.Editor.TrimByKeyword(GetElementName(element), keyWords, 16);
                return true;
            }
            else
            {
