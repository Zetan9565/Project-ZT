                        ...
                        textList = new ReorderableList(textsPaged, typeof(Text), true, true, false, false)
                        {
                            drawElementCallback = (rect, index, isActive, isFocused) =>
                            {
                                ...
                                EditorGUI.LabelField(new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, rect.width, EditorGUIUtility.singleLineHeight), 
                                    $"{ textsPaged[index].gameObject.GetPath()}");
                            },
                    ...
                    case ConvertType.TMP:
                        textList = new ReorderableList(textMeshsPaged, typeof(TextMeshProUGUI), true, true, false, false)
                        {
                            drawElementCallback = (rect, index, isActive, isFocused) =>
                            {
                                ...
                                EditorGUI.LabelField(new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, rect.width, EditorGUIUtility.singleLineHeight),
                                    $"{ textMeshsPaged[index].gameObject.GetPath()}");
                            },
                            elementHeightCallback = (index) =>
                            {
                                return EditorGUIUtility.singleLineHeight * 2;
                ...
                if (GUILayout.Button("一键收集"))
                {
                    switch (type)
                    {
                        case ConvertType.Legacy:
                            ...
                            if (!onlyOpenScene)
                            {
                                foreach (var go in ZetanEditorUtility.LoadAssets<GameObject>(path))
                                {
                ...
                switch (type)
                {
                    case ConvertType.Legacy:
                        if (font)
                        {
                            ...
                            textsPaged.Clear();
                            sw.Stop();
                            Debug.Log($"替换完成，耗时 {sw.ElapsedMilliseconds} ms");
                        }
