...
namespace ZetanStudio.Editor
{
    public class ConfigurationFinding : EditorWindow
    {
            ...
            foreach (var item in items)
            {
                ...
                switch (itemType)
                {
                    ...
                    case ItemKeyType.AsMater:
                        ...
                        break;
                    case ItemKeyType.Formu:
                        if (item.TryGetModule<CraftableModule>(out var makable) && typeof(CraftableModule).GetField("formulation", Utility.CommonBindingFlags).GetValue(makable).Equals(objKeys[0]))
                        {
                            take = true;
        ...
        private void SeekDialogue()
        {
            ...
            //            break;
            //        case DialogueKeyType.Words:
            //            int index = dialog.Contents.FindIndex(x => x is TextContent text && CompareString(text.Text, strKeys[0]));
            //            if (index >= 0)
            //            {
            //                take = true;
            //                if (strKeys[0].Length > 16 || type == SeekType.Equals)
